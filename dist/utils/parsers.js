...
exports.parsePreAuthorization = parsePreAuthorization;
exports.dictToCaptureResult = dictToCaptureResult;
exports.parseMandateBalanceResult = parseMandateBalanceResult;
exports.parseMandateRevokeResult = parseMandateRevokeResult;
...
function parseMandate(data) {
    ...
    return {
        ...
        amount: new types_1.Amount(amountToInt(amountValue), String(amountCurrency)),
...
function parsePreAuthorization(data) {
    const record = asRecord(data) ?? {};
    const customer = asRecord(record.customer) ?? {};
...
function parseMandateBalanceResult(data) {
    const record = asRecord(data) ?? {};
    const customer = asRecord(record.customer) ?? {};
    ...
    return {
        ...
        amount: asRecord(record.amount) ? new types_1.Amount(amountToInt(asRecord(record.amount)?.value), String(asRecord(record.amount)?.currency ?? "INR")) : undefined,
        ...
        balance_details: Object.keys(balanceDetails).length > 0 ? {
            amount_debited: new types_1.Amount(amountToInt(asRecord(balanceDetails.amount_debited)?.value), String(asRecord(balanceDetails.amount_debited)?.currency ?? "INR")),
            amount_remaining: new types_1.Amount(amountToInt(asRecord(balanceDetails.amount_remaining)?.value), String(asRecord(balanceDetails.amount_remaining)?.currency ?? "INR")),
        } : undefined,
        raw: record,
    };
}
function parseMandateRevokeResult(data) {
    const record = asRecord(data) ?? {};
    return {
        payment_method: normalizePaymentMethod(record.payment_method),
        payment_method_reference_id: String(record.payment_method_reference_id ?? ""),
...
function normalizePaymentMethod(value) {
    if (value === types_1.PaymentMethod.CARD) {
        return types_1.PaymentMethod.CARD;
    }
    if (value === types_1.PaymentMethod.CREDIT_EMI) {
        return types_1.PaymentMethod.CREDIT_EMI;
    }
    if (value === types_1.PaymentMethod.OTM) {
        return types_1.PaymentMethod.OTM;
    }
    return value === types_1.PaymentMethod.Crypto ? types_1.PaymentMethod.Crypto : types_1.PaymentMethod.RESERVE_PAY;
}
